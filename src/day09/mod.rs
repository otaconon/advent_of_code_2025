  ...
  match segment {
    AxisAlignedLine::Vertical { x, y_min, y_max } => {
      if *x > rect_x_min && *x < rect_x_max {
        let intersect_y_min = i64::max(*y_min, rect_y_min);
        let intersect_y_max = i64::min(*y_max, rect_y_max);
    ...
    AxisAlignedLine::Horizontal { y, x_min, x_max } => {
      if *y > rect_y_min && *y < rect_y_max {
        let intersect_x_min = i64::max(*x_min, rect_x_min);
        let intersect_x_max = i64::min(*x_max, rect_x_max);
  ...
  for (i, a) in points.iter().enumerate() {
    ...
    if a.x == b.x {
      segments.push(AxisAlignedLine::Vertical {
        x: a.x,
        y_min: i64::min(a.y, b.y),
        y_max: i64::max(a.y, b.y),
      })
    }
    if a.y == b.y {
      segments.push(AxisAlignedLine::Horizontal {
        y: a.y,
        x_min: i64::min(a.x, b.x),
        x_max: i64::max(a.x, b.x),
      })
    }
  }
  ...
  for a in &points {
    for b in &points {
      ...
      for segment in &segments {
        let x_min = i64::min(a.x, b.x);
        let x_max = i64::max(a.x, b.x);
        let y_min = i64::min(a.y, b.y);
        let y_max = i64::max(a.y, b.y);
